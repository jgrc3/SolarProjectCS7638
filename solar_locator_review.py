...
def estimate_next_pos(solar_system, gravimeter_measurement, other=None):
    ...
    samplePoints = []
    if not timer:
        # want to initialize 1000 random points
        for i in range(1000):
    ...
    # Step 2 - Assign importance weights - how close particle is to object - assign based on gravimeter measure
    # first need gravimeter measure
    # assign score to each point --  larger = better
    ...
    for i in range(len(resampledPoints)):
        ...
        for j in range(numberFuzz):
            # add fuzz -- pts closer get more fuzz
            radius = sqrt(resampledPoints[i].position[0]**2 + resampledPoints[i].position[1]**2)
            theta = atan2(resampledPoints[i].position[1], resampledPoints[i].position[0]) + random.uniform(-1*pi/8, pi/8)
            x = radius * cos(theta)
            y = radius * sin(theta)
