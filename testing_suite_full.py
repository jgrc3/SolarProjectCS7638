    ...
    def simulate_without_sos(self, estimate_next_pos, params):
        ...
        try:
            while steps < params['max_steps']:
                ...
                result = estimate_next_pos(solar_system=copy.deepcopy(solar_system), gravimeter_measurement=gravimeter_measurement, other=other_info)
                if len(result) == 3:
    ...
    def simulate_with_sos(self, next_angle, params):
        ...
        try:
            while steps < params['max_steps']:
                ...
                # Invoke student function to get student result
                gravimeter_measurement = target.sense(solar_system.planets)
                result = next_angle(solar_system=copy.deepcopy(solar_system), gravimeter_measurement=gravimeter_measurement, other=other_info)
                if len(result) == 4:
